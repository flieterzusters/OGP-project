...
import asteroids.model.programs.parsing.AsteroidsParserParser.ForloopContext;
...
public class AsteroidsParserMyListener<E, S, T> implements AsteroidsParserListener {
  ...
  private S StatementOfCtrl(AsteroidsParserParser.CtrlContext ctrl) {
    ...
    if(ctrl.forloop() != null) {
      return StatementOfForLoop(ctrl.forloop());
    }
  ...
  private S StatementOfForLoop(AsteroidsParserParser.ForloopContext w) {
    if(w == null)
      return null;
    int line = w.getStart().getLine();
    int column = w.getStart().getCharPositionInLine();
    ...
    return (factory.createForLoop(line, column, ExpressionOfExpr(c), StatementOfEval(e)));
  }
